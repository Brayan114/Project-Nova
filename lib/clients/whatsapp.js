...
export async function sendWhatsAppMessage(contactName, messageContent) {
    if (!whatsappClient) {
        throw new Error('WhatsApp Client is not initialized.');
    }

    try {
        console.log(`[NOVA WhatsApp Node] Searching for contact: ${contactName}`);
        
        // Find contact by name or phone
        const chats = await whatsappClient.getChats();
        const chat = chats.find(c => 
            (c.name && c.name.toLowerCase().includes(contactName.toLowerCase())) || 
            (c.id.user === contactName)
        );

        if (!chat) {
            throw new Error(`Contact "${contactName}" not found in your recent chats.`);
        }

        console.log(`[NOVA WhatsApp Node] Sending message to ${chat.name}: ${messageContent}`);
        await chat.sendMessage(messageContent);
        
        return {
            success: true,
            message: `Successfully sent WhatsApp message to ${chat.name}.`
        };

    } catch (error) {
        console.error('[NOVA WhatsApp Node] Send Error:', error.message);
