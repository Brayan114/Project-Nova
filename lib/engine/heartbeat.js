...
import { runAutonomousBrowsing } from './autonomous-browsing.js';
...
async function runHeartbeatTick() {
    ...
    for (const user of users) {
        ...
        // TRIGGER AUTONOMOUS BROWSING (15% chance per 15-min tick to feel natural)
        if (isAwakeTime && Math.random() < 0.15) {
            console.log(`[NOVA HEARTBEAT] Successful roll (15%)! Triggering autonomous browsing for ${user.id}...`);
            runAutonomousBrowsing(user.id).catch(err => {
                console.error(`[NOVA HEARTBEAT] Autonomous browsing task failed for ${user.id}:`, err);
            });
        }
    }
}
